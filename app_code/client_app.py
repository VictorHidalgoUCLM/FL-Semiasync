...
from app_code.task import (     # Importing auxiliary functions
    Net,
    load_data,
    get_weights,
    set_weights,
...
class FlowerClient(NumPyClient):
    ...
    def get_properties(self, config):
        return {"user": os.getenv("DEVICE")}    # Return client device name (e.g. "user": "supernode-1")
    ...
    def fit(self, parameters, config):
        ...
        if config.get('debug', False):
            ...
            return parameters, len(self.valloader.dataset), {
                "accuracy": 0.0, "loss": 0.0, "loss_distributed": 0.0, "precision": 0.0, "recall": 0.0, "f1": 0.0
            }

        set_weights(self.net, parameters)           # Set model weights to the ones received from the server
        ...
        if config.get('evaluate_on_fit', False):    # Evaluate the model if the flag is set, only for certain
            loss, _ = self.evaluate(parameters, {}) # strategies defined on the server

        else:                                       # If the flag is not set, return the training loss = 0.0
            loss = 0.0
    ...
    def evaluate(self, parameters, config):
        ...
        set_weights(self.net, parameters)   # Set model weights to the ones received from the server (aggregated weights)
        loss, accuracy, all_labels, all_preds = test(self.net, self.valloader, self.device) # Evaluate the model

        precision, recall, f1, accuracy = metrics_calculation(all_labels, all_preds)    # Calculate evaluation metrics
...
def client_fn(context: Context):
    ...
    trainloader, valloader = load_data(partition_id, num_partitions, partition_type)    # Create train and val loaders
